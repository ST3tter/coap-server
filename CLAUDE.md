...
Source and test files live in `src/`. Server logic is in `src/server.ts` (exports `server` and `handleRequest`), with `src/index.ts` as a thin entry point that calls `listen()`. Tests in `src/server.test.ts` start the server on port 5684 to avoid conflicts. ES module project running on Bun runtime.
